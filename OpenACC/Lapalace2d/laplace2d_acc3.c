...
void swap(double *__restrict A, double *__restrict A_new, int nx, int ny)
{
    // #pragma acc kernels loop independent copyin(A_new[:nx*ny]) copyout(A[:nx*ny]) collapse(2)
    #pragma acc parallel loop copyin(A_new[:nx*ny]) copyout(A[:nx*ny]) collapse(2)
    for (int i = 1; i < nx-1; i++)
    {
        for (int j = 1; j < ny-1; j++)
        {
