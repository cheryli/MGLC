...
void write(double *__restrict A, int nx, int ny);
...
double jacobi(double *restrict A, double *restrict A_new, int nx, int ny)
{
    double error = 0.0;
    #pragma acc kernels loop independent reduction(max:error) copyin(A[:nx*ny]) copy(A_new[:nx*ny]) 
...
void swap(double *restrict A, double *restrict A_new, int nx, int ny)
{
    #pragma acc kernels loop independent copyin(A_new[:nx*ny]) copyout(A[:nx*ny]) 
    for (int i = 1; i < nx-1; i++){
        for (int j = 1; j < ny-1; j++){
