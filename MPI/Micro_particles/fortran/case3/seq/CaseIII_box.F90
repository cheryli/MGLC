...
module commondata
    ...
    integer, parameter :: nx=201,ny=601
    ...
    real(8), parameter :: t0=5.0d0/100000.0d0 !! unit: s
    real(8), parameter :: m0=1.0d0/1000000.0d0 !! unit: g
    real(8), parameter :: tMax=1.2d0 !! unit: s
    ...
    real(8), parameter :: rhoSolid=1.25d0
    real(8) :: rhoAvg
    ...
    real(8), parameter :: thresholdR=2.0d0 
    real(8), parameter :: wallStiff=0.5d0
...
subroutine output_Tecplot()
    ...
    write(B2,'(i9.9)') itc
    open(41,file='particleBox-'//B2//'.plt', access='stream', form='unformatted')
...
subroutine updateCenter()
    ...
    open(unit=01,file='Y.dat',status='unknown',position='append')
    write(01,*) itc*t0, (yCenter(1)-0.5d0)*l0, yCenter(1)-radius0
    close(01)
    ...
    write(01,*) itc*t0, rhoSolid*2.0d0*radius0*Sqrt(Uc(1)**2.0d0+Vc(1)**2.0d0)/viscosity
    close(01)

    open(unit=01,file='Et.dat',status='unknown',position='append')
    write(01,*) itc*t0, 0.5d0*rhoSolid*Pi*radius0**2.0d0*(Uc(1)**2.0d0+Vc(1)**2.0d0)*m0*l0/t0**2.0d0
    close(01)
