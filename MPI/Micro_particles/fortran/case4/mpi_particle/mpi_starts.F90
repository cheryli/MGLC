subroutine mpi_starts()
    ...
    ! dims(0) = 1
    call MPI_Dims_create_2d(num_process, dims, total_nx, total_ny)
...
subroutine MPI_Dims_create_2d(num_process, dims, total_nx, total_ny)
    integer, intent(in) :: num_process, total_nx, total_ny
    integer, intent(inout) :: dims(0:1)
    integer :: i, j
    real :: message, diff

    diff = real(total_nx + total_ny) * real(num_process)
    do i = 1, num_process
        do j = 1, num_process
            if (i * j == num_process) then
                message = real(i - 1) * real(total_ny) + real(j - 1) * real(total_nx)
