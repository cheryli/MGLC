subroutine output()
    ...
    real(8), allocatable :: visual_rho(:, :)
    ! real(8), allocatable :: stream(:, :), vorticity(:, :)
    ! integer :: total_obst(:, :), tmp_obst(:, :)

    allocate(visual_rho(nx, ny))
    do j = 1, ny
        do i = 1, nx
            if (obst(i, j) .EQ. 0) then
                visual_rho(i, j) = 0.0d0
            else
                visual_rho(i, j) = 1.0d0
            endif
        enddo
    enddo
    ...
    if (rank2d > 0) then
        ...
        call MPI_Send(visual_rho, nx*ny, MPI_REAL8, 0, 3, comm2d, rc)
        ...
        do j = 1, ny
            do i = 1, nx
                ...
                total_rho(dx + i, dy + j) = visual_rho(i, j)
