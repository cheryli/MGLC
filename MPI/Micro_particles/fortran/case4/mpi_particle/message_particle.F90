...
subroutine update_particle_mask()
    use mpi
    use commondata
    implicit none
    integer :: cNum
    real(8) :: xCenter_mask(cNumMax), yCenter_mask(cNumMax)
    real(8) :: Uc_mask(cNumMax), Vc_mask(cNumMax), rationalOmega_mask(cNumMax)

    ! mask local particle
    do cNum = 1, cNumMax
            xCenter_mask(cNum) = real(local_mask(cNum)) * xCenter(cNum)
            yCenter_mask(cNum) = real(local_mask(cNum)) * yCenter(cNum)
            Uc_mask(cNum) = real(local_mask(cNum)) * Uc(cNum)   
            Vc_mask(cNum) = real(local_mask(cNum)) * Vc(cNum)
            rationalOmega_mask(cNum) = real(local_mask(cNum)) * rationalOmega(cNum) 
    enddo

    ! global sycronize    
    ! use all_reduce instead of broadcast
    ! only need 5 times instead of cNumMax times
    call MPI_ALLreduce(xCenter_mask, xCenter, cNumMax, MPI_DOUBLE_PRECISION, MPI_SUM, comm2d, rc)
    call MPI_ALLreduce(yCenter_mask, yCenter, cNumMax, MPI_DOUBLE_PRECISION, MPI_SUM, comm2d, rc)
    call MPI_ALLreduce(Uc_mask, Uc, cNumMax, MPI_DOUBLE_PRECISION, MPI_SUM, comm2d, rc)
    call MPI_ALLreduce(Vc_mask, Vc, cNumMax, MPI_DOUBLE_PRECISION, MPI_SUM, comm2d, rc)
    call MPI_ALLreduce(rationalOmega_mask, rationalOmega, cNumMax, MPI_DOUBLE_PRECISION, MPI_SUM, comm2d, rc)


end subroutine update_particle_mask