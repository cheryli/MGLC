...
subroutine collision()
    ...
    m(8) = f(5,i,j)-f(6,i,j)+f(7,i,j)-f(8,i,j)
            ...
            meq(0) = rho(i,j)
            meq(1) = -2.0d0 * rho(i,j) + 3.0d0*(u(i,j)*u(i,j)+v(i,j)*v(i,j))
            meq(2) = rho(i,j) - 3.0d0*(u(i,j)*u(i,j)+v(i,j)*v(i,j))
            meq(3) = u(i,j)
            meq(4) = -u(i,j)
            meq(5) = v(i,j)
            meq(6) = -v(i,j)
            meq(7) = u(i,j)*u(i,j)-v(i,j)*v(i,j) 
            meq(8) = u(i,j)*v(i,j) 
...
subroutine bounceback()
    ...
    do i=1,nx
        ...
        f(7,i,ny) = f_post(5,i,ny) - (Uwall(1))/6.0d0
        f(8,i,ny) = f_post(6,i,ny) - (-Uwall(1))/6.0d0
    enddo
...
subroutine macro()
    ...
    do j=1,ny
        do i=1,nx
            ...
            v(i,j) = ( f(2,i,j)-f(4,i,j)+f(5,i,j)+f(6,i,j)-f(7,i,j)-f(8,i,j) )
        enddo
    enddo
...
subroutine check()
    ...
    do j=1,ny
        do i=1,nx
            error1 = error1 + dsqrt((u(i,j)-up(i,j))*(u(i,j)-up(i,j))+(v(i,j)-vp(i,j))*(v(i,j)-vp(i,j)))
            error2 = error2 + dsqrt(u(i,j)*u(i,j)+v(i,j)*v(i,j))
