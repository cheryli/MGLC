...
subroutine message_passing()
    ...
    if(rank > 0) then
        ...
        call MPI_Send(fpost_btm_send, 3*nx, MPI_DOUBLE_PRECISION, rank-1, 0, MPI_COMM_WORLD, rc)
        call MPI_Recv(fpost_btm_receive, 3*nx, MPI_DOUBLE_PRECISION, rank-1, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE, rc)
    ...
    if(rank < num_process-1) then
        ...
        call MPI_Send(fpost_top_send, 3*nx, MPI_DOUBLE_PRECISION, rank+1, 1, MPI_COMM_WORLD, rc)
        call MPI_Recv(fpost_top_receive, 3*nx, MPI_DOUBLE_PRECISION, rank+1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE, rc)
