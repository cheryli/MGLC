subroutine message_passing()
    ...
    if (block_x > 1) then
        ...
        call MPI_Ssend(fps_send_i, 5*ny*nz, MPI_DOUBLE_PRECISION, rank-1, 1, MPI_COMM_WORLD, rc)
    ...
    if (block_y > 1) then
        ...
        call MPI_Ssend(fps_send_j, 5*nx*nz, MPI_DOUBLE_PRECISION, rank-nx_block, 3, MPI_COMM_WORLD, rc)
    ...
    if (block_y < ny_block) then
        ...
        call MPI_Ssend(fps_send_j, 5*nx*nz, MPI_DOUBLE_PRECISION, rank+nx_block, 4, MPI_COMM_WORLD, rc)
    ...
    if (block_z > 1) then
        ...
        call MPI_Ssend(fps_send_k, 5*nx*ny, MPI_DOUBLE_PRECISION, rank - nx_block*ny_block, 5, MPI_COMM_WORLD, rc)
    ...
    if (block_z < nz_block) then
        ...
        call MPI_Ssend(fps_send_k, 5*nx*ny, MPI_DOUBLE_PRECISION, rank + nx_block*ny_block, 6, MPI_COMM_WORLD, rc)
    ...
    if (block_x < nx_block .AND. block_y < ny_block) then
        ...
        call MPI_Ssend(fpl_send_z, nz, MPI_DOUBLE_PRECISION, rank + nx_block + 1, 7, MPI_COMM_WORLD, rc)
    ...
    if (block_x > 1 .AND. block_y < ny_block) then
        ...
        call MPI_Ssend(fpl_send_z, nz, MPI_DOUBLE_PRECISION, rank + nx_block - 1, 8, MPI_COMM_WORLD, rc)
    ...
    if (block_x < nx_block .AND. block_y > 1) then
        ...
        call MPI_Ssend(fpl_send_z, nz, MPI_DOUBLE_PRECISION, rank - nx_block + 1, 9, MPI_COMM_WORLD, rc)
    ...
    if (block_x > 1 .AND. block_y > 1) then
        ...
        call MPI_Ssend(fpl_send_z, nz, MPI_DOUBLE_PRECISION, rank - nx_block - 1, 10, MPI_COMM_WORLD, rc)
    ...
    if (block_x < nx_block .AND. block_z < nz_block) then
        ...
        call MPI_Ssend(fpl_send_y, ny, MPI_DOUBLE_PRECISION, rank + nx_block*ny_block + 1, 11, MPI_COMM_WORLD, rc)
    ...
    if (block_x > 1 .AND. block_z < nz_block) then
        ...
        call MPI_Ssend(fpl_send_y, ny, MPI_DOUBLE_PRECISION, rank + nx_block*ny_block - 1, 12, MPI_COMM_WORLD, rc)
    ...
    if (block_x < nx_block .AND. block_z > 1) then
        ...
        call MPI_Ssend(fpl_send_y, ny, MPI_DOUBLE_PRECISION, rank - nx_block*ny_block + 1, 13, MPI_COMM_WORLD, rc)
    ...
    if (block_x > 1 .AND. block_z > 1) then
        ...
        call MPI_Ssend(fpl_send_y, ny, MPI_DOUBLE_PRECISION, rank - nx_block*ny_block - 1, 14, MPI_COMM_WORLD, rc)
    ...
    if (block_y < ny_block .AND. block_z < nz_block) then
        ...
        call MPI_Ssend(fpl_send_x, nx, MPI_DOUBLE_PRECISION, rank + nx_block + nx_block*ny_block, 15, MPI_COMM_WORLD, rc)
    ...
    if (block_y > 1 .AND. block_z < nz_block) then
        ...
        call MPI_Ssend(fpl_send_x, nx, MPI_DOUBLE_PRECISION, rank - nx_block + nx_block*ny_block, 16, MPI_COMM_WORLD, rc)
    ...
    if (block_y < ny_block .AND. block_z > 1) then
        ...
        call MPI_Ssend(fpl_send_x, nx, MPI_DOUBLE_PRECISION, rank + nx_block - nx_block*ny_block, 17, MPI_COMM_WORLD, rc)
    ...
    if (block_y > 1 .AND. block_z > 1) then
        ...
        call MPI_Ssend(fpl_send_x, nx, MPI_DOUBLE_PRECISION, rank - nx_block - nx_block*ny_block, 18, MPI_COMM_WORLD, rc)
